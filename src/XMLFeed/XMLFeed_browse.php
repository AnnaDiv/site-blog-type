...
$postsStmt = $pdo->prepare('SELECT COUNT(*) as `count` FROM posts WHERE deleted = :deleted AND status = :status');
$postsStmt->bindValue(':deleted', false , PDO::PARAM_BOOL);
$postsStmt->bindValue(':status', 'public');
$postsStmt->execute();
...
$baseUrl = "http://localhost/blog-type-site/src/XMLFeed/XMLFeed_browse.php";
$paginationLinks = [
    'first' => $baseUrl . "?page=1",
    'prev'  => $baseUrl . "?page=" . ($page === 1 || $page < 1 ? 1 : $page - 1),
    'pages' => [],
    'next'  => $baseUrl . "?page=" . ($page == $num_pages ? $page : $page + 1),
    'last'  => $baseUrl . "?page=" . $num_pages
];
...
echo '<?xml-stylesheet type="text/xsl" href="feed-style-browse.xsl"?>';
?>
